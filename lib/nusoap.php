...
SoapSERVICES - Web Services Toolkit for PHP
...
The SoapSERVICES project home is:
http://sourceforge.net/projects/nusoap/

The primary support for SoapSERVICES is the Help forum on the project home page.
...
/*
 *	Some of the standards implmented in whole or part by SoapSERVICES:
 *
			...
			if ($this->methodparams) {
				foreach ($this->methodparams as $param) {
					if (is_array($param) || is_object($param)) {
						$this->fault('SOAP-ENV:Client', 'SoapSERVICES does not handle complexType parameters correctly when using eval; call_user_func_array must be available');
						return;
					}
					$funcCall .= "\"$param\",";
				}
...
class wsdl extends nusoap_base {
    ...
    */
    function webDescription(){
		...
		$b = '
		<html><head><title>SoapSERVICES: '.$this->serviceName.'</title>
		<style type="text/css">
			...
			if (is_array($parameters)) {
				...
				if ($style == 'document' && $use == 'literal' && $part_count == 1 && isset($parts['parameters'])) {
					...
					if ($direction == 'output' && $parametersArrayType == 'arraySimple' && $parameter_count == 1) {
						...
						// service function wraps, then SoapSERVICES wraps again
...
class nusoap_client extends nusoap_base  {
	...
	*
	* @return   string PHP/SoapSERVICES code for the proxy class
	* @access   private
	*/
